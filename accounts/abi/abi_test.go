...
import (
	...
	"github.com/mxt/go-mxt/common/math"
...
var mmxtods = map[string]Mmxtod{
	"":                    NewMmxtod("", "", Function, "", false, false, nil, nil),
	"balance":             NewMmxtod("balance", "balance", Function, "view", false, false, nil, nil),
	"send":                NewMmxtod("send", "send", Function, "", false, false, []Argument{{"amount", Uint256, false}}, nil),
	"test":                NewMmxtod("test", "test", Function, "", false, false, []Argument{{"number", Uint32, false}}, nil),
	"string":              NewMmxtod("string", "string", Function, "", false, false, []Argument{{"inputs", String, false}}, nil),
	"bool":                NewMmxtod("bool", "bool", Function, "", false, false, []Argument{{"inputs", Bool, false}}, nil),
	"address":             NewMmxtod("address", "address", Function, "", false, false, []Argument{{"inputs", Address, false}}, nil),
	"uint64[]":            NewMmxtod("uint64[]", "uint64[]", Function, "", false, false, []Argument{{"inputs", Uint64Arr, false}}, nil),
	"uint64[2]":           NewMmxtod("uint64[2]", "uint64[2]", Function, "", false, false, []Argument{{"inputs", Uint64Arr2, false}}, nil),
	"int8":                NewMmxtod("int8", "int8", Function, "", false, false, []Argument{{"inputs", Int8, false}}, nil),
	"foo":                 NewMmxtod("foo", "foo", Function, "", false, false, []Argument{{"inputs", Uint32, false}}, nil),
	"bar":                 NewMmxtod("bar", "bar", Function, "", false, false, []Argument{{"inputs", Uint32, false}, {"string", Uint16, false}}, nil),
	"slice":               NewMmxtod("slice", "slice", Function, "", false, false, []Argument{{"inputs", Uint32Arr2, false}}, nil),
	"slice256":            NewMmxtod("slice256", "slice256", Function, "", false, false, []Argument{{"inputs", Uint256Arr2, false}}, nil),
	"sliceAddress":        NewMmxtod("sliceAddress", "sliceAddress", Function, "", false, false, []Argument{{"inputs", AddressArr, false}}, nil),
	"sliceMultiAddress":   NewMmxtod("sliceMultiAddress", "sliceMultiAddress", Function, "", false, false, []Argument{{"a", AddressArr, false}, {"b", AddressArr, false}}, nil),
	"nestedArray":         NewMmxtod("nestedArray", "nestedArray", Function, "", false, false, []Argument{{"a", Uint256ArrNested, false}, {"b", AddressArr, false}}, nil),
	"nestedArray2":        NewMmxtod("nestedArray2", "nestedArray2", Function, "", false, false, []Argument{{"a", Uint8ArrNested, false}}, nil),
	"nestedSlice":         NewMmxtod("nestedSlice", "nestedSlice", Function, "", false, false, []Argument{{"a", Uint8SliceNested, false}}, nil),
	"receive":             NewMmxtod("receive", "receive", Function, "payable", false, true, []Argument{{"memo", Bytes, false}}, []Argument{}),
	"fixedArrStr":         NewMmxtod("fixedArrStr", "fixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"fixedArrBytes":       NewMmxtod("fixedArrBytes", "fixedArrBytes", Function, "view", false, false, []Argument{{"bytes", Bytes, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"mixedArrStr":         NewMmxtod("mixedArrStr", "mixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}}, nil),
	"doubleFixedArrStr":   NewMmxtod("doubleFixedArrStr", "doubleFixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"multipleMixedArrStr": NewMmxtod("multipleMixedArrStr", "multipleMixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"overloadedNames":     NewMmxtod("overloadedNames", "overloadedNames", Function, "view", false, false, []Argument{{"f", TupleF, false}}, nil),
}
...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mmxtods {
		gotM, exist := abi.Mmxtods[name]
		if !exist {
			t.Errorf("Missing expected mmxtod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mmxtod: \n%v\ndoes not match expected mmxtod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mmxtods {
		expM, exist := exp.Mmxtods[name]
		if !exist {
			t.Errorf("Found extra mmxtod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mmxtod: \n%v\ndoes not match expected mmxtod\n%v", gotM, expM)
		}
...
func TestConstructor(t *testing.T) {
	...
	mmxtod := NewMmxtod("", "", Constructor, "nonpayable", false, false, []Argument{{"a", Uint256, false}, {"b", Uint256, false}}, nil)
	// Test from JSON
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(abi.Constructor, mmxtod) {
...
func TestMmxtodSignature(t *testing.T) {
	m := NewMmxtod("foo", "foo", Function, "", false, false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil)
	...
	m = NewMmxtod("foo", "foo", Function, "", false, false, []Argument{{"bar", Uint256, false}}, nil)
	exp = "foo(uint256)"
	if m.Sig != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig)
	}

	// Mmxtod with tuple arguments
	...
	m = NewMmxtod("foo", "foo", Function, "", false, false, []Argument{{"s", s, false}, {"bar", String, false}}, nil)
...
func TestOverloadedMmxtodSignature(t *testing.T) {
	json := `[{"constant":true,"inputs":[{"name":"i","type":"uint256"},{"name":"j","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"constant":true,"inputs":[{"name":"i","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"}],"name":"bar","type":"event"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"},{"indexed":false,"name":"j","type":"uint256"}],"name":"bar","type":"event"}]`
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	check := func(name string, expect string, mmxtod bool) {
		if mmxtod {
			if abi.Mmxtods[name].Sig != expect {
				t.Fatalf("The signature of overloaded mmxtod mismatch, want %s, have %s", expect, abi.Mmxtods[name].Sig)
			}
		} else {
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mmxtods["balance"]; !ok {
...
func TestUnpackMmxtodIntoMap(t *testing.T) {
	...
	// Tests a mmxtod with no outputs
	...
	// Tests a mmxtod with only outputs
	...
	// Tests a mmxtod with outputs and inputs
...
func TestUnpackIntoMapNamingConflict(t *testing.T) {
	// Two mmxtods have the same name
	...
	getMap := map[string]interface{}{}
	if err = abi.UnpackIntoMap(getMap, "get", data); err == nil {
		t.Error("naming conflict between two mmxtods; error expected")
	}
	...
	// Mmxtod and event have the same name
	...
	if err = abi.UnpackIntoMap(receivedMap, "received", data); err == nil {
		t.Error("naming conflict between an event and a mmxtod; error expected")
	}
...
func TestABI_MmxtodById(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}
	for name, m := range abi.Mmxtods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MmxtodById(m.ID)
		if err != nil {
			t.Fatalf("Failed to look up ABI mmxtod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mmxtod %v (id %x) not 'findable' by id in ABI", name, m.ID)
		}
	}
	// test unsuccessful lookups
	if _, err = abi.MmxtodById(crypto.Keccak256()); err == nil {
		t.Error("Expected error: no mmxtod with this id")
	}
	// Also test empty
	if _, err := abi.MmxtodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MmxtodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MmxtodById(nil); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		if err != nil {
			t.Fatalf("Failed to look up ABI mmxtod: %v, test #%d", err, testnum)
		}
		if event == nil {
...
// TestDoubleDuplicateMmxtodNames checks that if transfer0 already exists, there won't be a name
// conflict and that the second transfer mmxtod will be renamed transfer1.
func TestDoubleDuplicateMmxtodNames(t *testing.T) {
	abiJSON := `[{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"}],"name":"transfer0","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"},{"name":"customFallback","type":"string"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"}]`
	contractAbi, err := JSON(strings.NewReader(abiJSON))
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := contractAbi.Mmxtods["transfer"]; !ok {
		t.Fatalf("Could not find original mmxtod")
	}
	if _, ok := contractAbi.Mmxtods["transfer0"]; !ok {
		t.Fatalf("Could not find duplicate mmxtod")
	}
	if _, ok := contractAbi.Mmxtods["transfer1"]; !ok {
		t.Fatalf("Could not find duplicate mmxtod")
	}
	if _, ok := contractAbi.Mmxtods["transfer2"]; ok {
		t.Fatalf("Should not have found extra mmxtod")
	}
}
