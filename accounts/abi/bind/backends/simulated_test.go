...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != mxt.NotFound {
		t.Fatalf("err should be `mxt.NotFound` but received %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		...
		{"plain transfer(valid)", mxt.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", mxt.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", mxt.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithVeryHighPrice", mxt.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1mxter
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1mxter
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", mxt.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2mxter
			Value:    big.NewInt(1000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2mxter-1000wei)/(2e14)
	}
	for _, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, mxt.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	if err != nil {
		t.Errorf("could not call receive mmxtod on contract: %v", err)
	}
	if len(res) == 0 {
	...
	res, err = sim.CallContract(bgCtx, mxt.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
contract Reverter {
    ...
    function noRevert() public pure {
        assembly {
            // Assembles sommxting that looks like require(false, "some error") but is not reverted
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, mxt.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, mxt.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
