...
func TestPack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			inDef := fmt.Sprintf(`[{ "name" : "mmxtod", "type": "function", "inputs": %s}]`, test.def)
			inAbi, err := JSON(strings.NewReader(inDef))
			if err != nil {
				t.Fatalf("invalid ABI definition %s, %v", inDef, err)
			}
			var packed []byte
			packed, err = inAbi.Pack("mmxtod", test.unpacked)
...
func TestMmxtodPack(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}

	sig := abi.Mmxtods["slice"].ID
	...
	sig = abi.Mmxtods["sliceAddress"].ID
	...
	sig = abi.Mmxtods["sliceMultiAddress"].ID
	...
	sig = abi.Mmxtods["nestedArray"].ID
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{1}, 32)...)
	sig = append(sig, common.LeftPadBytes([]byte{2}, 32)...)
	...
	sig = abi.Mmxtods["nestedArray2"].ID
	...
	sig = abi.Mmxtods["nestedSlice"].ID
	sig = append(sig, common.LeftPadBytes([]byte{0x20}, 32)...)
