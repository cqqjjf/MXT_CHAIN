...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			//Unpack
			def := fmt.Sprintf(`[{ "name" : "mmxtod", "type": "function", "outputs": %s}]`, test.def)
			...
			out, err := abi.Unpack("mmxtod", encb)
			if err != nil {
...
func TestLocalUnpackTests(t *testing.T) {
	for i, test := range unpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			err = abi.UnpackIntoInterface(outptr.Interface(), "mmxtod", encb)
...
type mmxtodMultiOutput struct {
	Int    *big.Int
	String string
}

func mmxtodMultiReturn(require *require.Assertions) (ABI, []byte, mmxtodMultiOutput) {
	const definition = `[
	{ "name" : "multi", "type": "function", "outputs": [ { "name": "Int", "type": "uint256" }, { "name": "String", "type": "string" } ] }]`
	var expected = mmxtodMultiOutput{big.NewInt(1), "hello"}
...
func TestMmxtodMultiReturn(t *testing.T) {
	...
	abi, data, expected := mmxtodMultiReturn(require.New(t))
	bigint := new(big.Int)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				require.Nil(err, "Should be able to unpack mmxtod outputs.")
...
func TestMultiReturnWithStringSlice(t *testing.T) {
	...
	ret1, ret1Exp := new([]string), []string{"mxt", "go-mxt"}
...
func TestUnpackTuple(t *testing.T) {
	...
	type T struct {
		X *big.Int `abi:"x"`
		Z *big.Int `abi:"y"` // Test whmxter the abi tag works.
	}
...
func TestOOMMaliciousInput(t *testing.T) {
	...
	for i, test := range oomTests {
		...
		_, err = abi.Mmxtods["mmxtod"].Outputs.UnpackValues(encb)
		if err == nil {
