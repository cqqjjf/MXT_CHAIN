...
type Wallet struct {
	...
	deriveChain     mxt.ChainStateReader // Blockchain state reader to discover used account with
...
// particular wallet instance. The reason there is no passwordless open mmxtod
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here because we don't know whmxter
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whmxter
...
// Contains returns whmxter an account is part of this particular wallet or not.
...
// this mmxtod supports providing multiple bases to discover old user accounts
// too. Only the last base will be used to derive the next empty account.
//
// You can disable automatic account discovery by calling SelfDerive with a nil
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain mxt.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whmxter the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whmxter the card has been initialized with a private key
}
