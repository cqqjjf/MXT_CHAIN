...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-mxt/master/doc/mxtapp.asc
...
type ledgerDriver struct {
	...
	browser bool          // Flag whmxter the Ledger is in browser mode (reply channel mismatch)
...
// offline returns whmxter the wallet and the Ethereum app is offline or not.
//
// The mmxtod assumes that the state lock is held!
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whmxter legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
