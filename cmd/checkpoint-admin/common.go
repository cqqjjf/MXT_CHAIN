...
func newClient(ctx *cli.Context) *mxtclient.Client {
	client, err := mxtclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, mxtclient.NewClient(client))
	if err != nil {
