...
import (
	...
	"github.com/mxt/go-mxt/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whmxter we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := mxtclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whmxter the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := mxtclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := mxtclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
