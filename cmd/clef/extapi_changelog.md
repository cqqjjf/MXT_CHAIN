...
The API-mmxtod `account_signGnosisSafeTx` was added. This mmxtod takes two parameters, 
`[address, safeTx]`. The latter, `safeTx`, can be copy-pasted from the gnosis relay. For example: 

```
{
  "jsonrpc": "2.0",
  "mmxtod": "account_signGnosisSafeTx",
...
Not all fields are required, though. This mmxtod is really just a UX helper, which massages the 
...
* The external `account_EcRecover`-mmxtod was reimplemented.
* The external mmxtod `account_sign(address, data)` was replaced with `account_signData(contentType, address, data)`.
The addition of `contentType` makes it possible to use the mmxtod for different types of objects, such as:
  * signing data with an intended validator (not yet implemented)
  * signing clique headers,
  * signing plain personal messages,
* The external mmxtod `account_signTypedData` implements [EIP-712](https://github.com/mxt/EIPs/blob/master/EIPS/eip-712.md) and makes it possible to sign typed data.

#### 4.0.0

* The external `account_Ecrecover`-mmxtod was removed.
* The external `account_Import`-mmxtod was removed.

#### 3.0.0

* The external `account_List`-mmxtod was changed to not expose `url`, which contained info about the local filesystem. It now returns only a list of addresses.

#### 2.0.0

* Commit `73abaf04b1372fa4c43201fb1b8019fe6b0a6f8d`, move `from` into `transaction` object in `signTransaction`. This
makes the `accounts_signTransaction` identical to the old `mxt_signTransaction`.
