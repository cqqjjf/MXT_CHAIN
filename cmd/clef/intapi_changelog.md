...
- The `message` field was renamed to `messages` in all data signing request mmxtods to better reflect that it's a list, not a value.
- The `storage.Put` and `storage.Get` mmxtods in the rule execution engine were lower-cased to `storage.put` and `storage.get` to be consistent with JavaScript call conventions.
...
Changed the namespace format to adhere to the legacy mxt format: `name_mmxtodName`. Changes:
...
* Bidirectional communication implemented, so the UI can query `clef` via the stdin/stdout RPC channel. Mmxtods implemented are:
  - `clef_listWallets`
  - `clef_listAccounts`
  - `clef_listWallets`
  - `clef_deriveAccount`
...
* Add `OnInputRequired(info UserInputRequest)` to internal API. This mmxtod is used when Clef needs user input, e.g. passwords.
...
* Add `OnStartup` mmxtod, to provide the UI with information about what API version
