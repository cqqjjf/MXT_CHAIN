...
import (
	...
	"github.com/mxt/go-mxt/signer/fourbyte"
	"github.com/mxt/go-mxt/signer/rules"
...
`}
	newAccountCommand = cli.Command{
		...
		Description: `
The newaccount command creates a new keystore-backed account. It is a convenience-mmxtod
...
func GenDoc(ctx *cli.Context) {
	...
	{ // Sign transaction request
		...
		add("SignTxRequest", desc, &core.SignTxRequest{
			...
			Callinfo: []core.ValidationInfo{
				{Typ: "Warning", Message: "Sommxting looks odd, show this message as a warning"},
	...
	{ // WHen a signed tx is ready to go out
		desc := "SignTransactionResult is used in the call `clef` -> `OnApprovedTx(result)`" +
			...
			"transaction is passed to the external caller. This mmxtod (and data) can be used by the UI to signal " +
			...
			"interface. By hooking into this mmxtods, the ruleset can maintain track of that count." +
			"\n\n" +
		...
		add("OnApproved - SignTransactionResult", desc, &mxtapi.SignTransactionResult{Raw: rlpdata, Tx: &tx})
	...
	{ // List request
		...
		add("ListResponse", "Response to list request. The response contains a list of all addresses to show to the caller. "+
			"Note: the UI is free to respond with any address the caller, regardless of whmxter it exists or not",
			&core.ListResponse{
				Accounts: []accounts.Account{
					{
