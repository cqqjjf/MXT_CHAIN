...
// TestEthProtocolNegotiation tests whmxter the test suite
// can negotiate the highest mxt protocol in a status message exchange
func TestEthProtocolNegotiation(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			assert.Equal(t, tt.expected, uint32(tt.conn.mxtProtocolVersion))
		})
	}
}

// TestChain_GetHeaders tests whmxter the test suite can correctly
