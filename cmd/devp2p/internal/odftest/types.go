...
import (
	...
	"github.com/mxt/go-mxt/internal/utesting"
	"github.com/mxt/go-mxt/p2p"
	"github.com/mxt/go-mxt/p2p/rlpx"
...
// Status is the network packet for the status message for mxt/64 and later.
type Status struct {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		if c.mxtProtocolVersion == 0 {
			t.Fatalf("unexpected mxt protocol version")
		}
		return msg
	default:
...
// negotiateEthProtocol sets the Conn's mxt protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "mxt" {
			continue
		}
	...
	c.mxtProtocolVersion = highestEthVersion
}
...
loop:
	...
	// make sure mxt protocol version is set for negotiation
	if c.mxtProtocolVersion == 0 {
		t.Fatalf("mxt protocol version must be set in Conn")
	}
	// write status message to client
	status := Status{
		ProtocolVersion: uint32(c.mxtProtocolVersion),
		NetworkID:       1,
