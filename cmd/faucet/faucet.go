...
var (
	...
	mxtPortFlag = flag.Int("mxtport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ethereum protocol")
	statsFlag   = flag.String("mxtstats", "", "Ethstats network monitoring auth string")
...
var (
	mxter = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	faucet, err := newFaucet(genesis, *mxtPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type faucet struct {
	...
	client *mxtclient.Client   // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the mxtstats monitoring and reporting service'
	if stats != "" {
		if err := mxtstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := mxtclient.NewClient(api)
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, mxter),
		"funded":   nonce,
	...
	for {
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Sommxting funky happened, please open an issue at https://github.com/mxt/go-mxt/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), mxter)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, mxter)
			peers := f.stack.Server().PeerCount()
