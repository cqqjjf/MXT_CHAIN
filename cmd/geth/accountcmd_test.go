...
func TestAccountListEmpty(t *testing.T) {
	gmxt := runGmxt(t, "account", "list")
	gmxt.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gmxt := runGmxt(t, "account", "list", "--datadir", datadir)
	defer gmxt.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gmxt := runGmxt(t, "account", "new", "--lightkdf")
	defer gmxt.ExpectExit()
	gmxt.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gmxt := runGmxt(t, "account", "import", keyfile, "-password", passwordFile)
	defer gmxt.ExpectExit()
	gmxt.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "sommxting"}}
Repeat password: {{.InputLine "sommxting else"}}
...
func TestWalletImport(t *testing.T) {
	gmxt := runGmxt(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gmxt.ExpectExit()
	gmxt.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gmxt.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gmxt.StderrText(), m) {
...
// https://github.com/mxt/go-mxt/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gmxt.SetTemplateFunc("keypath", func(file string) string {
