...
// gmxt is the official command-line client for Ethereum.
package main
...
import (
	...
	"github.com/mxt/go-mxt/mxtclient"
	"github.com/mxt/go-mxt/internal/debug"
...
const (
	clientIdentifier = "gmxt" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-mxt command line interface")
...
func init() {
	// Initialize the CLI app and start Gmxt
	app.Action = gmxt
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2020 The go-mxt Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.LegacyTestnetFlag.Name):
		log.Info("Starting Gmxt on Ropsten testnet...")
	...
	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Gmxt on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting Gmxt on G"orli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gmxt in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gmxt on Ethereum mainnet...")
	}
...
// gmxt is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gmxt(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend mxtapi.Backend) {
	...
	// Create a client to interact with local gmxt node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	mxtClient := mxtclient.NewClient(rpcClient)
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, mxtClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		mxtBackend, ok := backend.(*mxt.EthAPIBackend)
		if !ok {
		...
		mxtBackend.TxPool().SetGasPrice(gasprice)
		// start mining
		...
		if err := mxtBackend.StartMining(threads); err != nil {
