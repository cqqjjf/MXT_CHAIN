...
import (
	...
	"github.com/mxt/go-mxt/cmd/utils"
...
var (
	...
	retestmxtCommand = cli.Command{
		Action:      utils.MigrateFlags(retestmxt),
		Name:        "retestmxt",
		Usage:       "Launches gmxt in retestmxt mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gmxt in retestmxt mode (no database, no network, only retestmxt RPC interface)`,
	}
)

type RetestmxtTestAPI interface {
...
type RetestmxtEthAPI interface {
...
type RetestmxtDebugAPI interface {
...
type RetestmxtAPI struct {
	mxtDb         mxtdb.Database
	db            state.Database
...
func (api *RetestmxtAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(mxtDb, genesis)
	if err != nil {
		return false, err
	}
	...
	case "NoProof", "NoReward":
		inner = mxtash.NewFaker()
	case "Ethash":
		inner = mxtash.New(mxtash.Config{
			CacheDir:         "mxtash",
			CachesInMem:      2,
			CachesOnDisk:     3,
	...
	blockchain, err := core.NewBlockChain(mxtDb, nil, chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.mxtDb)
...
func (api *RetestmxtAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestmxtAPI) MineBlocks(ctx context.Context, number uint64) (bool, error) {
...
func (api *RetestmxtAPI) currentNumber() uint64 {
...
func (api *RetestmxtAPI) mineBlock() error {
	number := api.currentNumber()
	parentHash := rawdb.ReadCanonicalHash(api.mxtDb, number)
	parent := rawdb.ReadBlock(api.mxtDb, parentHash, number)
	...
	// If we are care about TheDAO hard-fork check whmxter to override the extra-data or not
	if daoBlock := api.chainConfig.DAOForkBlock; daoBlock != nil {
		// Check whmxter the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whmxter we support or oppose the fork, override differently
...
func (api *RetestmxtAPI) importBlock(block *types.Block) error {
...
func (api *RetestmxtAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetestmxtAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestmxtAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
...
func (api *RetestmxtAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.mxtDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetestmxtAPI) BlockNumber(ctx context.Context) (uint64, error) {
	return api.currentNumber(), nil
}

func (api *RetestmxtAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestmxtAPI) GetBlockByHash(ctx context.Context, blockHash common.Hash, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestmxtAPI) AccountRange(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
...
func (api *RetestmxtAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
...
func (api *RetestmxtAPI) GetCode(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (hexutil.Bytes, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return nil, err
...
func (api *RetestmxtAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (uint64, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return 0, err
...
func (api *RetestmxtAPI) StorageRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	address common.Address,
	begin *math.HexOrDecimal256, maxResults uint64,
) (StorageRangeResult, error) {
...
func (api *RetestmxtAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Gmxt-" + params.VersionWithCommit(gitCommit, gitDate), nil
}

func retestmxt(ctx *cli.Context) error {
	log.Info("Welcome to retestmxt!")
	// register signer API with server
	var (
		extapiURL string
	)
	apiImpl := &RetestmxtAPI{}
	var testApi RetestmxtTestAPI = apiImpl
	var mxtApi RetestmxtEthAPI = apiImpl
	var debugApi RetestmxtDebugAPI = apiImpl
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "mxt", "debug", "web3"}, srv, false)
	if err != nil {
	...
	// start http server
	var RetestmxtHTTPTimeouts = rpc.HTTPTimeouts{
		ReadTimeout:  120 * time.Second,
		WriteTimeout: 120 * time.Second,
		IdleTimeout:  120 * time.Second,
	}
	httpEndpoint := fmt.Sprintf("%s:%d", ctx.GlobalString(utils.HTTPListenAddrFlag.Name), ctx.Int(rpcPortFlag.Name))
	httpServer, _, err := node.StartHTTPEndpoint(httpEndpoint, RetestmxtHTTPTimeouts, handler)
	if err != nil {
