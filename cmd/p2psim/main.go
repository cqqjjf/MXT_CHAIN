	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mmxtod> [<args>]",
					Usage:     "call a node RPC mmxtod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mmxtod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mmxtod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mmxtod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mmxtod string, args ...string) error {
	parts := strings.SplitN(mmxtod, "_", 2)
	namespace := parts[0]
	mmxtod = parts[1]
