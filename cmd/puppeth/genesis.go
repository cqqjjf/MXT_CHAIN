...
import (
	...
	math2 "github.com/mxt/go-mxt/common/math"
...
// almxtGenesisSpec represents the genesis specification format used by the
// C++ Ethereum implementation.
type almxtGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*almxtGenesisSpecAccount `json:"accounts"`
}

// almxtGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type almxtGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *almxtGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// almxtGenesisSpecBuiltin is the precompiled contract definition.
type almxtGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *almxtGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type almxtGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlmxtGenesisSpec converts a go-mxt genesis block into a Almxt-specific
// chain specification format.
func newAlmxtGenesisSpec(network string, genesis *core.Genesis) (*almxtGenesisSpec, error) {
	// Only mxtash is currently supported between go-mxt and almxt
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Almxt format
	spec := &almxtGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	// config but almxt will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(mxtash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &almxtGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &almxtGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &almxtGenesisSpecBuiltin{Name: "sha256",
		Linear: &almxtGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &almxtGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &almxtGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &almxtGenesisSpecBuiltin{Name: "identity",
		Linear: &almxtGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &almxtGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &almxtGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &almxtGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &almxtGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &almxtGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &almxtGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &almxtGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Almxt hardcoded the gas policy
		spec.setPrecompile(7, &almxtGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Almxt hardcoded the gas policy
		spec.setPrecompile(9, &almxtGenesisSpecBuiltin{
...
func (spec *almxtGenesisSpec) setPrecompile(address byte, data *almxtGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*almxtGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &almxtGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *almxtGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*almxtGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-mxt/pull/11039,
...
// newParityChainSpec converts a go-mxt genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only mxtash is currently supported between go-mxt and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(mxtash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(mxtash.ConstantinopleBlockReward)
...
// newPyEthereumGenesisSpec converts a go-mxt genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only mxtash is currently supported between go-mxt and pymxt
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
