...
// Tests the go-mxt to Almxt chainspec conversion for the Stureby testnet.
func TestAlmxtSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gmxt.json")
	if err != nil {
	...
	spec, err := newAlmxtGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_almxt.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &almxtGenesisSpec{}
...
// Tests the go-mxt to Parity chainspec conversion for the Stureby testnet.
