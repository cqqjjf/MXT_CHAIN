...
// returned from docker inspect, parsed into a form easily usable by puppmxt.
type containerInfos struct {
	running bool              // Flag whmxter the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whmxter there's a container running for the service
	...
	// Infos retrieved, parse the above into sommxting meaningful
