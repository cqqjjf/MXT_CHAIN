...
FROM puppmxt/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gmxt --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gmxt --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --mxtstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,mxt,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gmxt --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --mxtstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,mxt,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.mxtstats[:strings.Index(config.node.mxtstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":       info.node.mxtstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.mxt"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			mxtstats: infos.envvars["ETH_NAME"],
		},
