...
FROM mxt/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--mxtstats", "{{.Ethstats}}", "--mxtport", "{{.EthPort}}",     \
...
// checkFaucet does a health-check against a faucet server to verify whmxter
