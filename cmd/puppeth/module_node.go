...
{{end}}
RUN \
  echo 'gmxt --cache 512 init /genesis.json' > gmxt.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.mxt/keystore/ && cp /signer.json /root/.mxt/keystore/' >> gmxt.sh && \{{end}}
	echo $'exec gmxt --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --mxtstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.mxterbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gmxt.sh

ENTRYPOINT ["/bin/sh", "gmxt.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.mxt{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.mxtash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.mxterbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ethstats":  config.mxtstats,
		"Etherbase": config.mxterbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethashdir":  config.mxtashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Ethstats":   config.mxtstats[:strings.Index(config.mxtstats, ":")],
		"Etherbase":  config.mxterbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.mxtstats,
	}
	if info.gasTarget > 0 {
		...
		if info.mxterbase != "" {
			...
			report["Ethash directory"] = info.mxtashdir
			report["Miner account"] = info.mxterbase
		}
		if info.keyJSON != "" {
...
// whmxter it's running, and if yes, whmxter it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gmxt --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.mxt"],
		mxtashdir:  infos.volumes["/root/.mxtash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		mxtstats:   infos.envvars["STATS_NAME"],
		mxterbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
