...
FROM puppmxt/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gmxt --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gmxt --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --mxtstats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppmxtNetworkID/{{.NetworkID}}/g' dist/js/mxterwallet-master.js && \
	sed -i 's/PuppmxtNetwork/{{.Network}}/g'     dist/js/mxterwallet-master.js && \
	sed -i 's/PuppmxtDenom/{{.Denom}}/g'         dist/js/mxterwallet-master.js && \
	sed -i 's/PuppmxtHost/{{.Host}}/g'           dist/js/mxterwallet-master.js && \
	sed -i 's/PuppmxtRPCPort/{{.RPCPort}}/g'     dist/js/mxterwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whmxter
// it's running, and if yes, whmxter it's responsive.
