...
// config contains all the configurations needed by puppmxt that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	mxtstats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppmxt configs", "file", c.path, "err", err)
	}
}
...
// weird formats used by mxtstats, which compares IPs textually, not by value.
