...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"mxtstats", "explorer", "wallet", "faucet"} {
	...
	// If we have mxtstats running, ask whmxter to make the secret public or not
	if w.conf.mxtstats != "" {
		fmt.Println()
		fmt.Println("Include mxtstats secret on dashboard (y/n)? (default = yes)")
