...
func (w *wizard) deployFaucet() {
	...
	// Figure which virtual-host to deploy mxtstats on
	...
	fmt.Println()
	if infos.node.mxtstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.mxtstats = w.readString() + ":" + w.conf.mxtstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.mxtstats)
		infos.node.mxtstats = w.readDefaultString(infos.node.mxtstats) + ":" + w.conf.mxtstats
	}
