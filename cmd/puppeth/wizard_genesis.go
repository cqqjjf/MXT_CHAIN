...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of mxtash, we're pretty much done
...
// importGenesis imports a Gmxt genesis spec into puppmxt.
...
func (w *wizard) manageGenesis() {
	// Figure out whmxter to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-almxt.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppmxt and Gmxt
		gmxtJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gmxtJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gmxtJson)

		// Export the genesis spec used by Almxt (formerly C++ Ethereum)
		if spec, err := newAlmxtGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Almxt chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "almxt", spec)
		}
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not sommxting I can do")
		return
	}
}
