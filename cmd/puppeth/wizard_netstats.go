...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for mxtstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["mxtstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["mxtstats"] = infos.Report()
		mxtstats = infos.config
	}
