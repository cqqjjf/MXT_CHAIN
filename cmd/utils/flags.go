...
// Package utils contains internal helper functions for go-mxt commands.
package utils
...
import (
	...
	"github.com/mxt/go-mxt/common/fdlimit"
	...
	"github.com/mxt/go-mxt/mxt"
	"github.com/mxt/go-mxt/mxt/downloader"
	"github.com/mxt/go-mxt/mxt/gasprice"
	"github.com/mxt/go-mxt/mxtdb"
	"github.com/mxt/go-mxt/mxtstats"
	"github.com/mxt/go-mxt/graphql"
	"github.com/mxt/go-mxt/internal/mxtapi"
	"github.com/mxt/go-mxt/internal/flags"
	"github.com/mxt/go-mxt/les"
	"github.com/mxt/go-mxt/log"
	"github.com/mxt/go-mxt/metrics"
	"github.com/mxt/go-mxt/metrics/exp"
	"github.com/mxt/go-mxt/metrics/influxdb"
	"github.com/mxt/go-mxt/miner"
	"github.com/mxt/go-mxt/node"
	"github.com/mxt/go-mxt/p2p"
	"github.com/mxt/go-mxt/p2p/discv5"
	"github.com/mxt/go-mxt/p2p/enode"
	"github.com/mxt/go-mxt/p2p/nat"
	"github.com/mxt/go-mxt/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: mxt.DefaultConfig.NetworkId,
	}
	...
	defaultSyncMode = mxt.DefaultConfig.SyncMode
	...
	LightServeFlag = cli.IntFlag{
		...
		Value: mxt.DefaultConfig.LightServ,
	}
	LightIngressFlag = cli.IntFlag{
		Name:  "light.ingress",
		Usage: "Incoming bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value: mxt.DefaultConfig.LightIngress,
	}
	LightEgressFlag = cli.IntFlag{
		Name:  "light.egress",
		Usage: "Outgoing bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value: mxt.DefaultConfig.LightEgress,
	}
	LightMaxPeersFlag = cli.IntFlag{
		Name:  "light.maxpeers",
		Usage: "Maximum number of light clients to serve, or light servers to attach to",
		Value: mxt.DefaultConfig.LightPeers,
	}
	UltraLightServersFlag = cli.StringFlag{
		Name:  "ulc.servers",
		Usage: "List of trusted ultra-light servers",
		Value: strings.Join(mxt.DefaultConfig.UltraLightServers, ","),
	}
	UltraLightFractionFlag = cli.IntFlag{
		Name:  "ulc.fraction",
		Usage: "Minimum % of trusted ultra-light servers required to announce a new head",
		Value: mxt.DefaultConfig.UltraLightFraction,
	}
	...
	EthashCacheDirFlag = DirectoryFlag{
		Name:  "mxtash.cachedir",
		Usage: "Directory to store the mxtash verification caches (default = inside the datadir)",
	}
	EthashCachesInMemoryFlag = cli.IntFlag{
		Name:  "mxtash.cachesinmem",
		Usage: "Number of recent mxtash caches to keep in memory (16MB each)",
		Value: mxt.DefaultConfig.Ethash.CachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "mxtash.cachesondisk",
		Usage: "Number of recent mxtash caches to keep on disk (16MB each)",
		Value: mxt.DefaultConfig.Ethash.CachesOnDisk,
	}
	EthashCachesLockMmapFlag = cli.BoolFlag{
		Name:  "mxtash.cacheslockmmap",
		Usage: "Lock memory maps of recent mxtash caches",
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "mxtash.dagdir",
		Usage: "Directory to store the mxtash mining DAGs",
		Value: DirectoryString(mxt.DefaultConfig.Ethash.DatasetDir),
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "mxtash.dagsinmem",
		Usage: "Number of recent mxtash mining DAGs to keep in memory (1+GB each)",
		Value: mxt.DefaultConfig.Ethash.DatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "mxtash.dagsondisk",
		Usage: "Number of recent mxtash mining DAGs to keep on disk (1+GB each)",
		Value: mxt.DefaultConfig.Ethash.DatasetsOnDisk,
	}
	EthashDatasetsLockMmapFlag = cli.BoolFlag{
		Name:  "mxtash.dagslockmmap",
		Usage: "Lock memory maps for recent mxtash mining DAGs",
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: mxt.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: mxt.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: mxt.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: mxt.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: mxt.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: mxt.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: mxt.DefaultConfig.TxPool.Lifetime,
	}
	...
	CacheTrieJournalFlag = cli.StringFlag{
		...
		Value: mxt.DefaultConfig.TrieCleanCacheJournal,
	}
	CacheTrieRejournalFlag = cli.DurationFlag{
		Name:  "cache.trie.rejournal",
		Usage: "Time interval to regenerate the trie cache journal",
		Value: mxt.DefaultConfig.TrieCleanCacheRejournal,
	}
	...
	MinerGasTargetFlag = cli.Uint64Flag{
		...
		Value: mxt.DefaultConfig.Miner.GasFloor,
	}
	MinerGasLimitFlag = cli.Uint64Flag{
		Name:  "miner.gaslimit",
		Usage: "Target gas ceiling for mined blocks",
		Value: mxt.DefaultConfig.Miner.GasCeil,
	}
	MinerGasPriceFlag = BigFlag{
		Name:  "miner.gasprice",
		Usage: "Minimum gas price for mining a transaction",
		Value: mxt.DefaultConfig.Miner.GasPrice,
	}
	MinerEtherbaseFlag = cli.StringFlag{
		Name:  "miner.mxterbase",
	...
	MinerRecommitIntervalFlag = cli.DurationFlag{
		...
		Value: mxt.DefaultConfig.Miner.Recommit,
	}
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in mxt_call/estimateGas (0=infinite)",
		Value: mxt.DefaultConfig.RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in mxter) that can be sent via the RPC APIs (0 = no cap)",
		Value: mxt.DefaultConfig.RPCTxFeeCap,
	}
	// Logging and debug settings
	EthStatsURLFlag = cli.StringFlag{
		Name:  "mxtstats",
		Usage: "Reporting URL of a mxtstats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: mxt.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpo.percentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: mxt.DefaultConfig.GPO.Percentile,
	}
	GpoMaxGasPriceFlag = cli.Int64Flag{
		Name:  "gpo.maxprice",
		Usage: "Maximum gas price will be recommended by gpo",
		Value: mxt.DefaultConfig.GPO.MaxPrice.Int64(),
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older Gmxt configurations storing the
...
// mmxtod returns nil and an emphemeral key is to be generated.
...
func setLes(ctx *cli.Context, cfg *mxt.Config) {
	...
	if cfg.UltraLightFraction <= 0 && cfg.UltraLightFraction > 100 {
		log.Error("Ultra light fraction is invalid", "had", cfg.UltraLightFraction, "updated", mxt.DefaultConfig.UltraLightFraction)
		cfg.UltraLightFraction = mxt.DefaultConfig.UltraLightFraction
	}
...
// for Gmxt and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gmxt account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setEtherbase retrieves the mxterbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setEtherbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *mxt.Config) {
	// Extract the current mxterbase, new flag overriding legacy one
	var mxterbase string
	if ctx.GlobalIsSet(LegacyMinerEtherbaseFlag.Name) {
		mxterbase = ctx.GlobalString(LegacyMinerEtherbaseFlag.Name)
		log.Warn("The flag --mxterbase is deprecated and will be removed in the future, please use --miner.mxterbase")

	}
	if ctx.GlobalIsSet(MinerEtherbaseFlag.Name) {
		mxterbase = ctx.GlobalString(MinerEtherbaseFlag.Name)
	}
	// Convert the mxterbase into an address and configure it
	if mxterbase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, mxterbase)
			if err != nil {
				Fatalf("Invalid miner mxterbase: %v", err)
			}
			cfg.Miner.Etherbase = account.Address
		} else {
			Fatalf("No mxterbase configured")
		}
	}
}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	mxtPeers := cfg.MaxPeers - lightPeers
	if lightClient {
		mxtPeers = 0
	}
	log.Info("Maximum peer count", "ETH", mxtPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setGPO(ctx *cli.Context, cfg *gasprice.Config, light bool) {
	...
	if light {
		cfg.Blocks = mxt.DefaultLightGPOConfig.Blocks
		cfg.Percentile = mxt.DefaultLightGPOConfig.Percentile
	}
...
func setEthash(ctx *cli.Context, cfg *mxt.Config) {
...
func setWhitelist(ctx *cli.Context, cfg *mxt.Config) {
...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/mxt/whisper")
	}
}

// SetEthConfig applies mxt-related command line flags to the config.
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *mxt.Config) {
...
// no URLs are set.
func SetDNSDiscoveryDefaults(cfg *mxt.Config, genesis common.Hash) {
...
func RegisterEthService(stack *node.Node, cfg *mxt.Config) mxtapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
...
// the given node.
func RegisterEthStatsService(stack *node.Node, backend mxtapi.Backend, url string) {
	if err := mxtstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Ethereum Stats service: %v", err)
	}
}

// RegisterGraphQLService is a utility function to construct a new service and register it against a node.
func RegisterGraphQLService(stack *node.Node, backend mxtapi.Backend, cfg node.Config) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gmxt.", tagsMap)
		}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) mxtdb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = makeDatabaseHandles()

		err     error
		chainDb mxtdb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb mxtdb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
	...
	if config.Clique != nil {
		...
		engine = mxtash.NewFaker()
		if !ctx.GlobalBool(FakePoWFlag.Name) {
			engine = mxtash.New(mxtash.Config{
				CacheDir:         stack.ResolvePath(mxt.DefaultConfig.Ethash.CacheDir),
				CachesInMem:      mxt.DefaultConfig.Ethash.CachesInMem,
				CachesOnDisk:     mxt.DefaultConfig.Ethash.CachesOnDisk,
				CachesLockMmap:   mxt.DefaultConfig.Ethash.CachesLockMmap,
				DatasetDir:       stack.ResolvePath(mxt.DefaultConfig.Ethash.DatasetDir),
				DatasetsInMem:    mxt.DefaultConfig.Ethash.DatasetsInMem,
				DatasetsOnDisk:   mxt.DefaultConfig.Ethash.DatasetsOnDisk,
				DatasetsLockMmap: mxt.DefaultConfig.Ethash.DatasetsLockMmap,
			}, nil, false)
		}
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
	...
	cache := &core.CacheConfig{
		TrieCleanLimit:      mxt.DefaultConfig.TrieCleanCache,
		TrieCleanNoPrefetch: ctx.GlobalBool(CacheNoPrefetchFlag.Name),
		TrieDirtyLimit:      mxt.DefaultConfig.TrieDirtyCache,
		TrieDirtyDisabled:   ctx.GlobalString(GCModeFlag.Name) == "archive",
		TrieTimeLimit:       mxt.DefaultConfig.TrieTimeout,
		SnapshotLimit:       mxt.DefaultConfig.SnapshotCache,
	}
	if !ctx.GlobalIsSet(SnapshotFlag.Name) {
...
// new format.
//
// e.g. gmxt account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this mmxtod with:
//
// gmxt --keystore /tmp/mykeystore --lightkdf account new
//
