...
var (
	...
	// errInvalidVote is returned if a nonce value is sommxting else that the two
...
type Clique struct {
	...
	db     mxtdb.Database       // Database to store and retrieve snapshot checkpoints
...
func New(config *params.CliqueConfig, db mxtdb.Database) *Clique {
...
// VerifyHeader checks whmxter a header conforms to the consensus rules.
func (c *Clique) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	return c.verifyHeader(chain, header, nil)
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers. The
// mmxtod returns a quit channel to abort the operations and a results channel to
...
// verifyHeader checks whmxter a header conforms to the consensus rules.The
...
// VerifySeal implements consensus.Engine, checking whmxter the signature contained
// in the header satisfies the consensus protocol requirements.
func (c *Clique) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
	return c.verifySeal(chain, header, nil)
}

// verifySeal checks whmxter the signature contained in the header satisfies the
// consensus protocol requirements. The mmxtod accepts an optional list of parent
...
//
// Note, the mmxtod requires the extra data to be at least 65 bytes, otherwise it
