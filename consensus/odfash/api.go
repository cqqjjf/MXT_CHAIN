...
var errEthashStopped = errors.New("mxtash stopped")

// API exposes mxtash related mmxtods for the RPC interface.
type API struct {
	mxtash *Ethash
}
...
func (api *API) GetWork() ([4]string, error) {
	...
	select {
	case api.mxtash.remote.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.mxtash.remote.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.mxtash.remote.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashRate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.mxtash.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.mxtash.remote.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.mxtash.Hashrate())
}
