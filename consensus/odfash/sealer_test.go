...
import (
	...
	"github.com/mxt/go-mxt/internal/testlog"
	"github.com/mxt/go-mxt/log"
)

// Tests whmxter remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom mxtash engine.
	mxtash := NewTester([]string{server.URL}, false)
	defer mxtash.Close()
	...
	mxtash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := mxtash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom mxtash engine.
	mxtash := NewTester([]string{server.URL}, false)
	mxtash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer mxtash.Close()
	...
	for i := 0; i < cap(sink); i++ {
		...
		block := types.NewBlockWithHeader(header)
		mxtash.Seal(nil, block, results, nil)
	}
...
// Tests whmxter stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	mxtash := NewTester(nil, true)
	defer mxtash.Close()
	api := &API{mxtash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			mxtash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, mxtash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch mxtash result timeout", id+1)
		}
	}
}
