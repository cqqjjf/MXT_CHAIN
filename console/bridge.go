...
import (
	...
	"github.com/mxt/go-mxt/accounts/scwallet"
	"github.com/mxt/go-mxt/accounts/usbwallet"
	"github.com/mxt/go-mxt/common/hexutil"
	"github.com/mxt/go-mxt/console/prompt"
	"github.com/mxt/go-mxt/internal/jsre"
	"github.com/mxt/go-mxt/rpc"
)

// bridge is a collection of JavaScript utility mmxtods to bride the .js runtime
// environment and the Go RPC connection backing the remote mmxtod calls.
type bridge struct {
...
func getJmxt(vm *goja.Runtime) *goja.Object {
	jmxt := vm.Get("jmxt")
	if jmxt == nil {
		panic(vm.ToValue("jmxt object does not exist"))
	}
	return jmxt.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC mmxtod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mmxtod (saved in jmxt.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJmxt(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jmxt.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJmxt(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jmxt.openWallet is not callable")
	}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC mmxtod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mmxtod (saved in jmxt.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJmxt(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jmxt.unlockAccount is not callable")
	}
	return unlockAccount(goja.Null(), account, passwd, duration)
}

// Sign is a wrapper around the personal.sign RPC mmxtod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mmxtod (saved in
// jmxt.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJmxt(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jmxt.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "mxt_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "mxt_blockNumber"); err != nil {
			return nil, err
		}
...
// Send implements the web3 provider "send" mmxtod.
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.Mmxtod, req.Params...); err == nil {
			if result == nil {
