...
import (
	...
	"github.com/mxt/go-mxt/internal/jsre/deps"
	"github.com/mxt/go-mxt/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"mxt": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// initPersonal redirects account-related API mmxtods through the bridge.
//
// If the console is in interactive mode and the 'personal' API is available, override
// the openWallet, unlockAccount, newAccount and sign mmxtods since these require user
// interaction. The original web3 callbacks are stored in 'jmxt'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jmxt := vm.NewObject()
	vm.Set("jmxt", jmxt)
	jmxt.Set("openWallet", personal.Get("openWallet"))
	jmxt.Set("unlockAccount", personal.Get("unlockAccount"))
	jmxt.Set("newAccount", personal.Get("newAccount"))
	jmxt.Set("sign", personal.Get("sign"))
...
// consoleOutput is an override for the console.log and console.error mmxtods to
...
// input prompter to provide hints to the user about the mmxtods available.
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines mxt.getBalance(mxt.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
		// Skip all mmxtods and namespaces (i.e. including the dot)
...
// Welcome show summary of current Gmxt instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gmxt JavaScript console!\n\n"

	// Print some generic Gmxt metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + mxt.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + mxt.blockNumber + " (" + new Date(1000 * mxt.getBlock(mxt.blockNumber).timestamp) + ")\n";
		try {
