...
// ChainIndexerBackend defines the mmxtods needed to process chain segments in
...
type ChainIndexer struct {
	chainDb  mxtdb.Database      // Chain database to index the data from
	indexDb  mxtdb.Database      // Prefixed table-view of the db to write index metadata into
	backend  ChainIndexerBackend // Background processor generating the index data content
	children []*ChainIndexer     // Child indexers to cascade chain updates to

	active    uint32          // Flag whmxter the event loop was started
...
func NewChainIndexer(chainDb mxtdb.Database, indexDb mxtdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
