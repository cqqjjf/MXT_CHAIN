...
func SetupGenesisBlock(db mxtdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// Check whmxter the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) ToBlock(db mxtdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db mxtdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db mxtdb.Database) *types.Block {
...
func GenesisBlockForTesting(db mxtdb.Database, addr common.Address, balance *big.Int) *types.Block {
...
// DeveloperGenesisBlock returns the 'gmxt --dev' genesis block.
