...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(mxtdb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db mxtdb.Database) (*params.ChainConfig, common.Hash, error) {
		...
		},
		{
			...
			fn: func(db mxtdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, mxtash.NewFullFaker(), vm.Config{}, nil, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, mxtash.NewFaker(), db, 4, nil)
