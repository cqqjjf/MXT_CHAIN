...
// of frozen ancient blocks. The mmxtod iterates over all the frozen blocks and
// injects into the database the block hash->number mappings.
func InitDatabaseFromFreezer(db mxtdb.Database) {
...
func iterateTransactions(db mxtdb.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
...
// signal received.
func indexTransactions(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if from >= to {
		return
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db mxtdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
