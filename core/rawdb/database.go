...
import (
	...
	"github.com/mxt/go-mxt/mxtdb/leveldb"
	"github.com/mxt/go-mxt/mxtdb/memorydb"
...
// Freeze is a helper mmxtod used for external testing to trigger and block until
...
func NewDatabase(db mxtdb.KeyValueStore) mxtdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db mxtdb.KeyValueStore, freezer string, namespace string) (mxtdb.Database, error) {
	...
	// validate in this mmxtod. If, however, the genesis hash is not nil, compare
	// it to the freezer content.
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains sommxting, ensure that the genesis blocks
...
func NewMemoryDatabase() mxtdb.Database {
...
func NewMemoryDatabaseWithCap(size int) mxtdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (mxtdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (mxtdb.Database, error) {
...
func InspectDatabase(db mxtdb.Database) error {
