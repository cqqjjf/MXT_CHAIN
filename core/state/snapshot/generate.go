...
func (gs *generatorStats) Log(msg string, root common.Hash, marker []byte) {
	...
	// Figure out whmxter we're after or within an account
	switch len(marker) {
...
func generateSnapshot(diskdb mxtdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, wiper chan struct{}) *diskLayer {
...
// gmxtering and logging, since the mmxtod surfs the blocks as they arrive, often
// being restarted.
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	for accIt.Next() {
		...
		if batch.ValueSize() > mxtdb.IdealBatchSize || abort != nil {
			// Only write and set the marker if we actually did sommxting useful
