...
type journalGenerator struct {
	Wiping   bool // Whmxter the database was in progress of being wiped
	Done     bool // Whmxter the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadSnapshot(diskdb mxtdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash) (snapshot, error) {
	...
	// Retrieve the journal, it must exist since even for 0 layer it stores whmxter
	...
	if !generator.Done {
		...
		// Whmxter or not wiping was in progress, load any generator progress too
