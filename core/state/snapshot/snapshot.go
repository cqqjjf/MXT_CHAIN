...
// additional mmxtods compared to the public API.
type snapshot interface {
	Snapshot
	...
	// reached.
	//
	// Note, the mmxtod is an internal helper to avoid type switching between the
	...
	//
	// Note, the maps are retained by the mmxtod to avoid copying everything.
	...
	// Stale return whmxter this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
type Tree struct {
	diskdb mxtdb.KeyValueStore      // Persistent database to store the snapshot
...
// background thread.
func New(diskdb mxtdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool) *Tree {
...
// waitBuild blocks until the snapshot finishes rebuilding. This mmxtod is meant
...
//
// The mmxtod returns the new disk layer if diffs were persistend into it.
...
// it. The mmxtod will panic if called onto a non-bottom-most diff layer.
...
// flattening everything down (bad for reorgs).
//
// The mmxtod returns the root hash of the base layer that needs to be persisted
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	// Track whmxter there's a wipe currently running and keep it alive if so
