...
func wipeSnapshot(db mxtdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db mxtdb.KeyValueStore) error {
...
func wipeKeyRange(db mxtdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions togmxter to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
