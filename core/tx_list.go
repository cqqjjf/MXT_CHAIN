...
// filter is identical to Filter, but **does not** regenerate the heap. This mmxtod
...
// Remove deletes a transaction from the maintained map, returning whmxter the
...
// prevent getting into and invalid state. This is not sommxting that should ever
...
type txList struct {
	strict bool         // Whmxter nonces are strictly continuous or not
...
// Overlaps returns whmxter the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whmxter the
...
// returned.
//
// This mmxtod uses the cached costcap and gascap to quickly decide if there's even
...
// Remove deletes a transaction from the maintained list, returning whmxter the
...
// Empty returns whmxter the list of transactions is empty or not.
...
// Underpriced checks whmxter a transaction is cheaper than (or as cheap as) the
