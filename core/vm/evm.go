...
type Context struct {
	// CanTransfer returns whmxter the account contains
	// sufficient mxter to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers mxter from one account to the other
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) mxt virtual machine
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	// Note although it's noop to transfer X mxter to caller itself. But
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	// check whmxter the max code size has been exceeded
