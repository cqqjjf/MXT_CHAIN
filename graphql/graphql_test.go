...
func TestGraphQLHTTPOnSamePort_GQLRequest_Successful(t *testing.T) {
	...
	gqlReq, err := http.NewRequest(http.MmxtodGet, fmt.Sprintf("http://%s/graphql", "127.0.0.1:9393"), body)
	if err != nil {
...
func TestGraphQLHTTPOnSamePort_GQLRequest_Unsuccessful(t *testing.T) {
	...
	// create http request
	body := strings.NewReader("{\"query\": \"{block{number}}\",\"variables\": null}")
	gqlReq, err := http.NewRequest(http.MmxtodPost, fmt.Sprintf("http://%s/graphql", "127.0.0.1:9393"), body)
	if err != nil {
		t.Error("could not issue new http request ", err)
	}
	gqlReq.Header.Set("Content-Type", "application/json")
...
func createGQLService(t *testing.T, stack *node.Node, endpoint string) {
	// create backend
	mxtBackend, err := mxt.New(stack, &mxt.DefaultConfig)
	if err != nil {
		t.Fatalf("could not create mxt backend: %v", err)
	}

	// create gql service
	err = New(stack, mxtBackend.APIBackend, []string{}, []string{})
	if err != nil {
