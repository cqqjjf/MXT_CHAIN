...
// Package mxt defines interfaces for interacting with Ethereum.
package mxt
...
// NotFound is returned by API mmxtods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mmxtods in this interface access raw
...
type ChainReader interface {
	...
	// This mmxtod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mmxtod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mmxtod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by mxt_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mmxtod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mmxtod can be used to retrieve the
