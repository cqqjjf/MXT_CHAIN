...
type TestCmd struct {
	// For total convenience, all testing mmxtods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gmxt-test" in cmd/gmxt/run_test.go)
...
// This mmxtod can also be called from an expect template, e.g.:
//
//     gmxt.expect(`Passphrase: {{.InputLine "password"}}`)
