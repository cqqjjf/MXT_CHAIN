...
import (
	...
	"github.com/mxt/go-mxt/p2p/simulations"
...
// similar requests at the maximum allowed rate and the test verifies whmxter the
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "mxt_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "mxt_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	mxt, err := mxt.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, mxt, &config)
	if err != nil {
		return nil, err
	}
	return mxt, nil
