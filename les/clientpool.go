...
// and negative banalce. Bomxt positive balance and negative balance will decrease
...
func newClientPool(ns *nodestate.NodeStateMachine, lespayDb mxtdb.Database, minCap uint64, connectedBias time.Duration, clock mclock.Clock, removePeer func(enode.ID)) *clientPool {
	pool := &clientPool{
		ns:                  ns,
