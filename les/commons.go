...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, mxtconfig *mxt.Config) *checkpointoracle.CheckpointOracle {
	config := mxtconfig.CheckpointOracle
	if config == nil {
	...
	client := mxtclient.NewClient(rpcClient)
	oracle.Start(client)
