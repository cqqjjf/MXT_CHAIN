...
// the mxt.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb mxtdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		syncing      bool        // Indicator whmxter the client is syncing
	...
	// trustedHeader returns an indicator whmxter the header is regarded as
...
//
// Note, we rely on the underlying mxt/fetcher to retrieve and validate the
// response, so that we have to obey the rule of mxt/fetcher which only accepts
