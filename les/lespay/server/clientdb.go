...
type nodeDB struct {
	...
	evictCallBack func(mclock.AbsTime, bool, utils.ExpiredValue) bool // Callback to determine whmxter the balance can be evicted.
	clock         mclock.Clock
	closeCh       chan struct{}
	cleanupHook   func() // Test hook used for testing
}

func newNodeDB(db mxtdb.KeyValueStore, clock mclock.Clock) *nodeDB {
...
// expireNodes iterates the whole node db and checks whmxter the
