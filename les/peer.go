...
import (
	...
	lps "github.com/mxt/go-mxt/les/lespay/server"
	"github.com/mxt/go-mxt/les/utils"
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whmxter the peer is frozen.
...
// canQueue returns an indicator whmxter the peer can queue an operation.
...
func (p *peerCommons) Info() *mxt.PeerInfo {
	return &mxt.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whmxter the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whmxter the server sends announcement only.
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64, bool) bool // Used to determine whmxter the server has the specified block.
}
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local mxt node is running in archive mode, advertise ourselves we have
