...
func newPruner(db mxtdb.Database, indexers ...*core.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
...
// historical chain data. Notably, whenever Gmxt restarts, it will iterate
...
func (p *pruner) loop() {
	...
	// Note, if some indexers don't support pruning(e.g. mxt.BloomIndexer),
