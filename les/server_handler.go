...
const (
	...
	mxtVersion        = 63              // equivalent mxt version for the downloader
...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whmxter local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb mxtdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light mxt response", "err", err)
			return err
		default:
		}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	// accept returns an indicator whmxter the request can be served.
