...
import (
	...
	"github.com/mxt/go-mxt/accounts/abi/bind/backends"
	"github.com/mxt/go-mxt/common"
	"github.com/mxt/go-mxt/common/mclock"
	"github.com/mxt/go-mxt/consensus/mxtash"
	"github.com/mxt/go-mxt/contracts/checkpointoracle/contract"
	...
	"github.com/mxt/go-mxt/les/checkpointoracle"
	"github.com/mxt/go-mxt/les/flowcontrol"
	"github.com/mxt/go-mxt/light"
	"github.com/mxt/go-mxt/p2p"
	"github.com/mxt/go-mxt/p2p/enode"
	"github.com/mxt/go-mxt/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some mxter to user1
		...
		case 1:
			...
			// bankUser transfers more mxter to user1
		...
		case 2:
			// bankUser transfer some mxter to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db mxtdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = mxt.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db mxtdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightEthereum{
		lesCommons: lesCommons{
			...
			config:      &mxt.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db mxtdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
