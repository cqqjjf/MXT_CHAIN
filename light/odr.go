...
func (req *TrieRequest) StoreResult(db mxtdb.Database) {
	req.Proof.Store(db)
}
...
// StoreResult stores the retrieved data in local database
func (req *CodeRequest) StoreResult(db mxtdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BlockRequest) StoreResult(db mxtdb.Database) {
	rawdb.WriteBodyRLP(db, req.Hash, req.Number, req.Rlp)
}

// ReceiptsRequest is the ODR request type for retrieving receipts.
type ReceiptsRequest struct {
	Untrusted bool // Indicator whmxter the result retrieved is trusted or not
	Hash      common.Hash
	Number    uint64
	Header    *types.Header
...
// StoreResult stores the retrieved data in local database
func (req *ReceiptsRequest) StoreResult(db mxtdb.Database) {
	if !req.Untrusted {
...
// StoreResult stores the retrieved data in local database
func (req *ChtRequest) StoreResult(db mxtdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BloomRequest) StoreResult(db mxtdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *TxStatusRequest) StoreResult(db mxtdb.Database) {}
