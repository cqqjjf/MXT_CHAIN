...
import (
	...
	"github.com/mxt/go-mxt/common/bitutil"
...
func GetChtRoot(db mxtdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db mxtdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    mxtdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db mxtdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
...
func GetBloomTrieRoot(db mxtdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db mxtdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db mxtdb.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *core.ChainIndexer {
