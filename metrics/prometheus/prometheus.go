...
// Package prommxteus exposes go-metrics into a Prommxteus format.
package prommxteus
...
// Handler returns an HTTP handler which dump metrics in Prommxteus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metris into a Prommxteus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown Prommxteus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
