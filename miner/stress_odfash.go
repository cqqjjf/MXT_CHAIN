...
func main() {
	...
	// Pre-generate the mxtash mining DAG so we don't race
	mxtash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".mxtash"))
	...
	for i := 0; i < 4; i++ {
		...
		stack, mxtBackend, err := makeMiner(genesis)
		if err != nil {
			panic(err)
		}
		defer stack.Close()
...
func makeMiner(genesis *core.Genesis) (*node.Node, *mxt.Ethereum, error) {
	...
	mxtBackend, err := mxt.New(stack, &mxt.Config{
		Genesis:         genesis,
		...
		GPO:             mxt.DefaultConfig.GPO,
		Ethash:          mxt.DefaultConfig.Ethash,
		Miner: miner.Config{
