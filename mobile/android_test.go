...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gmxt.LightScryptN, Gmxt.LightScryptP);
		...
		try {
			...
			// data from this mmxtod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.mxt", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-mxt issue #14599.
...
//
// This mmxtod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogmxter
	...
	workspace, err := ioutil.TempDir("", "gmxt-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/mxt/gmxttest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gmxt and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.mxt", "github.com/mxt/go-mxt/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gmxt.aar"), "gmxt.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "mxt", "gmxttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
