...
// mmxtod to sign the transaction before submission.
...
// NewKeyedTransactOpts is a utility mmxtod to easily create a transaction signer
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, mmxtod string, args *Interfaces) error {
	results := make([]interface{}, len(out.objects))
	copy(results, out.objects)
	if err := c.contract.Call(&opts.opts, &results, mmxtod, args.objects...); err != nil {
		return err
	}
...
func (c *BoundContract) Transact(opts *TransactOpts, mmxtod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, mmxtod, args.objects...)
	if err != nil {
		return nil, err
	}
...
// RawTransact invokes the (paid) contract mmxtod with raw calldata as input values.
