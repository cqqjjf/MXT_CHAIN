...
// NodeConfig represents the collection of configuration values to fine tune the Gmxt
// node embedded into a mobile process. The available values are a subset of the
// entire API provided by go-mxt to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whmxter the node should run the Ethereum protocol.
...
// Node represents a Gmxt Ethereum node instance.
type Node struct {
	node *node.Node
}

// NewNode creates and configures a new Gmxt node.
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumEnabled {
		mxtConf := mxt.DefaultConfig
		mxtConf.Genesis = genesis
		mxtConf.SyncMode = downloader.LightSync
		mxtConf.NetworkId = uint64(config.EthereumNetworkID)
		mxtConf.DatabaseCache = config.EthereumDatabaseCache
		lesBackend, err := les.New(rawStack, &mxtConf)
		if err != nil {
			return nil, fmt.Errorf("mxt init: %v", err)
		}
		// If netstats reporting is requested, do it
		if config.EthereumNetStats != "" {
			if err := mxtstats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.EthereumNetStats); err != nil {
...
func (n *Node) GetEthereumClient() (client *EthereumClient, _ error) {
	...
	return &EthereumClient{mxtclient.NewClient(rpc)}, nil
}
