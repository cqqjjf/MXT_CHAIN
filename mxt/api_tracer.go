...
import (
	...
	"github.com/mxt/go-mxt/mxt/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.mxt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.mxt.blockchain.CurrentBlock()
	default:
		from = api.mxt.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.mxt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.mxt.blockchain.CurrentBlock()
	default:
		to = api.mxt.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.mxt.ChainDb(), 16, "") // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.mxt.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.mxt.blockchain.Config(), task.block.Number())

				// Trace all the transactions contained within
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(signer)
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.mxt.blockchain, nil)
					...
					task.statedb.Finalise(api.mxt.blockchain.Config().IsEIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.mxt.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.mxt.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.mxt.blockchain.Config().IsEIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.mxt.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.mxt.blockchain.CurrentBlock()
	default:
		block = api.mxt.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.mxt.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceBadBlock(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	blocks := api.mxt.blockchain.BadBlocks()
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.mxt.engine.VerifyHeader(api.mxt.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.mxt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		signer = types.MakeSigner(api.mxt.blockchain.Config(), block.Number())
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.mxt.blockchain, nil)
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, api.mxt.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(vmctx, statedb, api.mxt.blockchain.Config(), vmConf)
...
// containsTx reports whmxter the transaction with a certain hash
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.mxt.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	// Otherwise try to reexec blocks until we find a state or reach our limit
	origin := block.NumberU64()
	database := state.NewDatabaseWithCache(api.mxt.ChainDb(), 16, "")

	for i := uint64(0); i < reexec; i++ {
		block = api.mxt.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.mxt.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.mxt.ChainDb(), hash)
	if tx == nil {
	...
	// Retrieve the block
	block := api.mxt.blockchain.GetBlockByHash(blockHash)
	if block == nil {
...
// TraceCall lets you trace a given mxt_call. It collects the structured logs created during the execution of EVM
// if the given transaction was added on top of the provided block and returns them as a JSON object.
// You can provide -2 as a block number to trace on top of the pending block.
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args mxtapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	// First try to retrieve the state
	statedb, header, err := api.mxt.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.mxt.APIBackend.RPCGasCap())
	vmctx := core.NewEVMContext(msg, header, api.mxt.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, api.mxt.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
	...
	case *vm.StructLogger:
		...
		return &mxtapi.ExecutionResult{
			Gas:         result.UsedGas,
			Failed:      result.Failed(),
			ReturnValue: returnVal,
			StructLogs:  mxtapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.mxt.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, statedb, api.mxt.blockchain.Config(), vm.Config{})
