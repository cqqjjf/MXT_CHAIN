...
func (mxt *Ethereum) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-mxt.closeBloomHandler:
					return

				case request := <-mxt.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(mxt.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(mxt.chainDb, task.Bit, section, head); err == nil {
...
type BloomIndexer struct {
	...
	db      mxtdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db mxtdb.Database, size, confirms uint64) *core.ChainIndexer {
