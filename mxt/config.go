...
func init() {
	...
	if runtime.GOOS == "darwin" {
		...
		DefaultConfig.Ethash.DatasetDir = filepath.Join(home, ".mxtash")
	}
}
...
type Config struct {
	...
	NoPruning  bool // Whmxter to disable pruning and flush everything to disk
	NoPrefetch bool // Whmxter to disable prefetching and only load state on demand
	...
	LightNoPrune bool `toml:",omitempty"` // Whmxter to disable light chain pruning

	// Ultra Light client options
	UltraLightServers      []string `toml:",omitempty"` // List of trusted ultra light servers
	UltraLightFraction     int      `toml:",omitempty"` // Percentage of trusted servers to accept an announcement
	UltraLightOnlyAnnounce bool     `toml:",omitempty"` // Whmxter to only announce headers, or also serve them
	...
	// RPCGasCap is the global gas cap for mxt-call variants.
	RPCGasCap uint64 `toml:",omitempty"`

	// RPCTxFeeCap is the global transaction fee(price * gaslimit) cap for
	// send-transction variants. The unit is mxter.
