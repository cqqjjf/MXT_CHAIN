...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [mxt/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    mxtdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [mxt/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [mxt/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [mxt/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [mxt/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [mxt/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [mxt/62] Channel to feed the header processor new tasks
	...
	stateCh        chan dataPack // [mxt/63] Channel receiving inbound node state data
	...
	// Testing hooks
	syncInitHook     func(uint64, uint64)  // Mmxtod to call upon initiating a new sync run
	bodyFetchHook    func([]*types.Header) // Mmxtod to call upon starting a block body fetch
	receiptFetchHook func([]*types.Header) // Mmxtod to call upon starting a receipt fetch
	chainInsertHook  func([]*fetchResult)  // Mmxtod to call upon inserting a chain of blocks (possibly in multiple invocations)
}
...
func New(checkpoint uint64, stateDb mxtdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() mxt.SyncProgress {
...
// Synchronising returns whmxter the downloader is currently retrieving blocks.
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		if d.dropPeer == nil {
			// The dropPeer mmxtod is nil when `--copydb` is used for a local copy.
...
// checks fail an error will be returned. This mmxtod is synchronous
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "mxt", p.version, "head", hash, "td", td, "mode", mode)
...
// not wait for the running download goroutines to finish. This mmxtod should be
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave sommxting valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whmxter the next request is pivot verification
...
//
// The mmxtod returns the entire filled skeleton and also the number of headers
...
// types, this mmxtod is used by each for data gathering and is instrumented with
...
//  - expire:      task callback mmxtod to abort requests that took too long and return the faulty peers (traffic shaping)
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered sommxting completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have sommxting)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us sommxting useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if sommxting failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The InsertChain mmxtod in blockchain.go will sometimes return an out-of-bounds index,
			// when it needs to preprocess blocks to import a sidechain.
			// The importer will put togmxter a new list of blocks to import, which is a superset
