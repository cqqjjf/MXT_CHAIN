...
type downloadTester struct {
	...
	stateDb mxtdb.Database // Database used by the tester for syncing from peers
	peerDb  mxtdb.Database // Database of the peers containing all data
...
// This mmxtod assumes that the caller holds at least the read-lock (dl.lock)
func (dl *downloadTester) getHeaderByHash(hash common.Hash) *types.Header {
...
// RequestBodies constructs a getBlockBodies mmxtod associated with a particular
...
// RequestReceipts constructs a getReceipts mmxtod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
// RequestNodeData constructs a getNodeData mmxtod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	// Mark this mmxtod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", mxt.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", mxt.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", mxt.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", mxt.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want mxt.SyncProgress) {
	// Mark this mmxtod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", mxt.SyncProgress{
