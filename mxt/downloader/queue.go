...
import (
	...
	"github.com/mxt/go-mxt/common/prque"
...
type fetchRequest struct {
	...
	From    uint64          // [mxt/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [mxt/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [mxt/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [mxt/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [mxt/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [mxt/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [mxt/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [mxt/62] Result cache accumulating the completed headers
	headerProced    int                            // [mxt/62] Number of headers already processed from the results
	headerOffset    uint64                         // [mxt/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [mxt/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [mxt/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [mxt/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [mxt/62] Currently pending block (body) retrieval operations

	receiptTaskPool  map[common.Hash]*types.Header // [mxt/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [mxt/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [mxt/63] Currently pending receipt retrieval operations
...
// InFlightHeaders retrieves whmxter there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whmxter there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whmxter there are receipt fetch requests currently
// in flight.
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading sommxting (sanity check to
	// not corrupt state)
...
// returns a flag whmxter empty blocks were queued requiring processing.
...
// also returns a flag whmxter empty receipts were queued requiring importing.
...
// skipping any previously failed ones. This mmxtod is a generic version used
// by the individual special reservation functions.
//
// Note, this mmxtod expects the queue lock to be already held for writing. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
//
// Returns:
//   item     - the fetchRequest
//   progress - whmxter any progress was made
//   throttle - if the caller should throttle for a while
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, kind uint) (*fetchRequest, bool, bool) {
	// Short circuit if the pool has been depleted, or if the peer's already
	// downloading sommxting (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
// Revoke cancels all pending requests belonging to a given peer. This mmxtod is
...
//
// Note, this mmxtod expects the queue lock to be already held. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
...
// cache. This mmxtod either accepts all headers it received, or none of them
// if they do not map correctly to the skeleton.
//
// If the headers are accepted, the mmxtod makes an attempt to deliver the set
...
// The mmxtod returns the number of blocks bodies accepted from the delivery and
...
// The mmxtod returns the number of transaction receipts accepted from the delivery
