...
// stateReq represents a batch of state fetch requests grouped togmxter into
...
type stateReq struct {
	...
	dropped   bool                      // Flag whmxter the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting sommxting else, ignore the stale timeout.
...
// newStateSync creates a new state trie download scheduler. This mmxtod does not
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for sommxting to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < mxtdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under mxt/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
...
// delivered. Returns whmxter the peer actually managed to deliver anything of
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for hash, task := range req.trieTasks {
		// If the node did deliver sommxting, missing items may be due to a protocol
...
// peer into the state trie, returning whmxter anything useful was written or any
// error occurred.
