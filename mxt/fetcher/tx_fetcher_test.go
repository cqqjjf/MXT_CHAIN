...
func TestTransactionFetcherWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Initial announcement to get sommxting into the waitlist
...
func TestTransactionFetcherOutOfBoundDeliveries(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Deliver sommxting out of the blue
			isWaiting(nil),
			isScheduled{nil, nil, nil},
...
func testTransactionFetcher(t *testing.T, tt txFetcherTest) {
	...
	for i, step := range tt.steps {
		...
		case doWait:
			clock.Run(step.time)
			if step.step {
				<-wait // Fetcher supposed to do sommxting, wait until it's done
			}
...
// containsHash returns whmxter a hash is contained within a hash slice.
