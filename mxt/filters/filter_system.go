...
// Package filters implements an mxt filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mmxtod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mmxtod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mmxtod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit mxt.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit mxt.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit mxt.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit mxt.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
