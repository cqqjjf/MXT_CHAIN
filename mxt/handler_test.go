...
// Tests that post mxt protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	blockchain, err := core.NewBlockChain(db, nil, config, mxtash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create new blockchain: %v", err)
	}
	pm, err := NewProtocolManager(config, cht, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, mxtash.NewFaker(), blockchain, db, 1, nil)
	if err != nil {
	...
	peer, _ := newTestPeer("peer", mxt63, pm, true)
	defer peer.close()
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	for i := 0; i < totalPeers; i++ {
		peer, _ := newTestPeer(fmt.Sprintf("peer %d", i), mxt63, pm, true)
		defer peer.close()

		peers = append(peers, peer)
	}
	chain, _ := core.GenerateChain(gspec.Config, genesis, mxtash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {})
...
func TestBroadcastMalformedBlock(t *testing.T) {
	...
	// propagation
	source, _ := newTestPeer("source", mxt63, pm, true)
	defer source.close()

	sink, _ := newTestPeer("sink", mxt63, pm, true)
	defer sink.close()
