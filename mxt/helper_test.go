...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, mxtdb.Database, error) {
	var (
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, mxtash.NewFaker(), db, blocks, generator)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, mxtdb.Database) {
...
// Has returns an indicator whmxter txpool has a transaction
...
func (p *testPeer) handshake(t *testing.T, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) {
	...
	default:
		panic(fmt.Sprintf("unsupported mxt protocol version: %d", p.version))
	}
