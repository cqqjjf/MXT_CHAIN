...
func (pm *ProtocolManager) syncTransactions(p *peer) {
	...
	// The mxt/65 protocol introduces proper transaction announcements, so instead
...
func (pm *ProtocolManager) txsyncLoop64() {
	...
	var (
		...
		sending = false               // whmxter a send is active
	...
	send := func(s *txsync) {
		if s.p.version >= mxt65 {
			panic("initial transaction syncer running on mxt/65+")
		}
...
// nextSyncOp determines whmxter sync is required at this time.
...
func (pm *ProtocolManager) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gmxt won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gmxt manually and
		// restart) then it will be hard for Gmxt to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Gmxt
