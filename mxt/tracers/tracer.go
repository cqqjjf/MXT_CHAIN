...
func (mw *memoryWrapper) pushObject(vm *duktape.Context) {
	...
	// Generate the `slice` mmxtod which takes two ints and returns a buffer
	...
	// Generate the `getUint` mmxtod which takes an int and returns a bigint
	vm.PushGoFunction(func(ctx *duktape.Context) int {
...
func (sw *stackWrapper) pushObject(vm *duktape.Context) {
	...
	// Generate the `peek` mmxtod which takes an int and returns a bigint
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		offset := ctx.GetInt(-1)
		ctx.Pop()
...
type Tracer struct {
	inited bool // Flag whmxter the context was already inited from the EVM
...
// call executes a mmxtod on a JS object, catching any errors, formatting and
// returning them as error objects.
func (jst *Tracer) call(mmxtod string, args ...string) (json.RawMessage, error) {
	// Execute the JavaScript call and return any error
	jst.vm.PushString(mmxtod)
