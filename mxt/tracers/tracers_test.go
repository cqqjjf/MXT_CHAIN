...
import (
	...
	"github.com/mxt/go-mxt/tests"
)

// To generate a new callTracer test, copy paste the makeTest mmxtod below into
// a Gmxt console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = mxt.getBlock(mxt.getTransaction(tx).blockHash);
  var genesis = mxt.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.mxt.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  mxt.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
