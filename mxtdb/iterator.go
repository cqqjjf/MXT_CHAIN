...
// value pairs. The error can be queried by calling the Error mmxtod. Calling
...
// Iteratee wraps the NewIterator mmxtods of a backing data store.
type Iteratee interface {
	// NewIterator creates a binary-alphabetical iterator over a subset
	// of database content with a particular key prefix, starting at a particular
	// initial key (or after, if it does not exist).
	//
	// Note: This mmxtod assumes that the prefix is NOT part of the start, so there's
