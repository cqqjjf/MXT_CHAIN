...
// New returns a wrapped map with all the required database interface mmxtods
// implemented.
func New() *Database {
	return &Database{
...
// all the required database interface mmxtods implemented.
...
func (db *Database) NewBatch() mxtdb.Batch {
	return &batch{
		db: db,
	}
}
...
func (db *Database) NewIterator(prefix []byte, start []byte) mxtdb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
//
// Note, this mmxtod is only used for testing (i.e. not public in general) and
...
func (b *batch) Replay(w mxtdb.KeyValueWriter) error {
	for _, keyvalue := range b.writes {
		if keyvalue.delete {
...
// Next moves the iterator to the next key/value pair. It returns whmxter the
