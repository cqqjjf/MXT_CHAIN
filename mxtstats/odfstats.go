...
// Package mxtstats implements the network stats reporting service.
package mxtstats
...
// backend encompasses the bare-minimum functionality needed for mxtstats reporting
...
//   Applications are responsible for ensuring that no more than one goroutine calls the write mmxtods
//     - NextWriter, SetWriteDeadline, WriteMessage, WriteJSON, EnableWriteCompression, SetCompressionLevel
//   concurrently and that no more than one goroutine calls the read mmxtods
//     - NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPongHandler, SetPingHandler
//   concurrently.
//   The Close and WriteControl mmxtods can be called concurrently with all other mmxtods.
...
// WriteJSON wraps corresponding mmxtod on the websocket but is safe for concurrent calling
...
// ReadJSON wraps corresponding mmxtod on the websocket but is safe for concurrent calling
...
// Close wraps corresponding mmxtod on the websocket but is safe for concurrent calling
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(mxtstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["mxt"]; info != nil {
		network = fmt.Sprintf("%d", info.(*mxt.NodeInfo).Network)
		protocol = fmt.Sprintf("mxt/%d", mxt.ProtocolVersions[0])
	} else {
...
// server. Use the individual mmxtods for reporting subscribed events.
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the mxtstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to mxtstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to mxtstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to mxtstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to mxtstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to mxtstats")
