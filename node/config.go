...
import (
	...
	"github.com/mxt/go-mxt/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gmxt is "gmxt". If no
	...
	// registered services, instead those can use utility mmxtods to create/access
...
// ExtRPCEnabled returns the indicator whmxter node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gmxt", keep that.
	if name == "gmxt" || name == "gmxt-testnet" {
		name = "Gmxt"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gmxt" instances.
var isOldGmxtResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gmxt 1.4 are used if they exist.
	if warn, isOld := isOldGmxtResource[path]; isOld {
		oldpath := ""
		if c.name() == "gmxt" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGmxtResourceWarning, "Using deprecated resource file %s, please move this file to the 'gmxt' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-mxt-keystore")
		ephemeral = keydir
	}
