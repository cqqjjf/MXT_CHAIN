...
import (
	...
	"github.com/prommxteus/tsdb/fileutil"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mmxtod works before the node has started. We rely on
	// this in cmd/gmxt.
...
// configureRPC is a helper mmxtod to configure all the various RPC endpoints during node
...
// Server retrieves the currently running P2P network layer. This mmxtod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (mxtdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (mxtdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
// closeTrackingDB wraps the Close mmxtod of a database. When the database is closed by the
...
func (n *Node) wrapDatabase(db mxtdb.Database) mxtdb.Database {
