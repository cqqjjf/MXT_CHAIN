...
import (
	...
	"github.com/mxt/go-mxt/p2p/discover/v4wire"
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whmxter the callback should
...
type reply struct {
	...
	ip   net.IP
	data v4wire.Packet
	// loop indicates whmxter there was
...
// whmxter any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whmxter any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
type packetHandlerV4 struct {
	...
	// preverify checks whmxter the packet is valid and should be handled at all.
