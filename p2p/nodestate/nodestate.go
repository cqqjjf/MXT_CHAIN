...
type (
	...
	// function description, along with whmxter it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  mxtdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whmxter the operands belong to the same setup
...
func NewNodeStateMachine(db mxtdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whmxter the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whmxter the field belongs to the same setup and returns its internal index
...
// checkStarted checks whmxter the state machine has already been started and panics otherwise.
...
// opCheck checks whmxter an operation is active
