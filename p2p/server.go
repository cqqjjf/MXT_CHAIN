...
import (
	...
	"github.com/mxt/go-mxt/p2p/discover"
...
type Config struct {
	...
	// DiscoveryV5 specifies whmxter the new topic-discovery based V5 discovery
...
//
// This mmxtod blocks until all protocols have exited and the peer is removed. Do not use
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "mxt discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "mxt p2p")
				srv.loopWG.Done()
			}()
		}
	}
