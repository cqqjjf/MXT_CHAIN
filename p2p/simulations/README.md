...
creating a collection of devp2p nodes, connecting them togmxter to form a
...
if a node is created without an explicit service), exposes mmxtods for
...
connecting two nodes togmxter).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of sommxting happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mmxtod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whmxter or not a
...
On return, the `Simulation.Run` mmxtod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mmxtod> [<args>] [--subscribe]
```
