...
import (
	...
	"github.com/mxt/go-mxt/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togmxter.
//
...
// Connect connects two nodes togmxter by calling the "admin_addPeer" RPC
// mmxtod on the "one" node so that it connects to the "other" node
...
// mmxtod on the "one" node so that it disconnects from the "other" node
...
// GetRandomNode returns a random node on the network, regardless of whmxter it is running or not
...
// NOTE:
// it also checks whmxter there has been recent attempt to connect the peers
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		// If the node has the close mmxtod, call it.
...
type Node struct {
	...
	// up tracks whmxter or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whmxter the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/mxtersphere/go-mxt/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
...
type Conn struct {
	...
	// Up tracks whmxter or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whmxter both nodes are currently up
