...
func confirmRequestValidationCode(t *testing.T, mmxtod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	request := httptest.NewRequest(mmxtod, "http://url.com", strings.NewReader(body))
...
func TestHTTPErrorResponseWithDelete(t *testing.T) {
	confirmRequestValidationCode(t, http.MmxtodDelete, contentType, "", http.StatusMmxtodNotAllowed)
}

func TestHTTPErrorResponseWithPut(t *testing.T) {
	confirmRequestValidationCode(t, http.MmxtodPut, contentType, "", http.StatusMmxtodNotAllowed)
}

func TestHTTPErrorResponseWithMaxContentLength(t *testing.T) {
	body := make([]rune, maxRequestContentLength+1)
	confirmRequestValidationCode(t,
		http.MmxtodPost, contentType, string(body), http.StatusRequestEntityTooLarge)
}

func TestHTTPErrorResponseWithEmptyContentType(t *testing.T) {
	confirmRequestValidationCode(t, http.MmxtodPost, "", "", http.StatusUnsupportedMediaType)
}

func TestHTTPErrorResponseWithValidRequest(t *testing.T) {
	confirmRequestValidationCode(t, http.MmxtodPost, contentType, "", 0)
}

func confirmHTTPRequestYieldsStatusCode(t *testing.T, mmxtod, contentType, body string, expectedStatusCode int) {
	t.Helper()
	s := Server{}
	ts := httptest.NewServer(&s)
	defer ts.Close()

	request, err := http.NewRequest(mmxtod, ts.URL, strings.NewReader(body))
	if err != nil {
...
func TestHTTPResponseWithEmptyGet(t *testing.T) {
	confirmHTTPRequestYieldsStatusCode(t, http.MmxtodGet, "", "", http.StatusOK)
}
