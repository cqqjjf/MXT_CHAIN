...
func TestSubscriptions(t *testing.T) {
	...
	for i, namespace := range namespaces {
		request := map[string]interface{}{
			"id":      i,
			"mmxtod":  fmt.Sprintf("%s_subscribe", namespace),
			"version": "2.0",
...
func TestServerUnsubscribe(t *testing.T) {
	...
	p2.Write([]byte(`{"jsonrpc":"2.0","id":1,"mmxtod":"nftest2_subscribe","params":["someSubscription",0,10]}`))
	...
	p2.Write([]byte(`{"jsonrpc":"2.0","mmxtod":"nftest2_unsubscribe","params":["` + sub.subid + `"]}`))
	for {
		select {
