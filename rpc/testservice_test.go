...
func (s *testService) CallMeBack(ctx context.Context, mmxtod string, args []interface{}) (interface{}, error) {
	c, ok := ClientFromContext(ctx)
	if !ok {
		return nil, errors.New("no client")
	}
	var result interface{}
	err := c.Call(&result, mmxtod, args...)
	return result, err
}

func (s *testService) CallMeBackLater(ctx context.Context, mmxtod string, args []interface{}) error {
	c, ok := ClientFromContext(ctx)
	if !ok {
...
func (s *notificationTestService) SomeSubscription(ctx context.Context, n, val int) (*Subscription, error) {
	...
	// events might be send before the response for the *_subscribe mmxtod.
