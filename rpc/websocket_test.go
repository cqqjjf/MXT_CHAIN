...
// This test checks whmxter calls exceeding the request size limit are rejected.
...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/mxt/go-mxt/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the mxt_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","mmxtod":"mxt_subscription","params":{"subscription":"0x00","result":1}}`
	)
