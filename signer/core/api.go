...
import (
	...
	"github.com/mxt/go-mxt/signer/storage"
)
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mmxtodSelector *string) (*mxtapi.SignTransactionResult, error)
	...
	SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, mmxtodSelector *string) (*GnosisSafeTx, error)
}

// UIClientAPI specifies what mmxtod a UI needs to implement to be able to be used as a
// UI for the signer
type UIClientAPI interface {
	...
	// This mmxtod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx mxtapi.SignTransactionResult)
...
// Validator defines the mmxtods required to validate a transaction against some
// sanity defaults as well as any underlying 4byte mmxtod database.
//
...
// newAccount is the internal mmxtod to create a new account. It should be used
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or sommxting
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mmxtodSelector *string) (*mxtapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(mmxtodSelector, &args)
	if err != nil {
		return nil, err
	}
	...
	response := mxtapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
func (api *SignerAPI) SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, mmxtodSelector *string) (*GnosisSafeTx, error) {
...
// Returns the external api version. This mmxtod does not require user acceptance. Available mmxtods are
