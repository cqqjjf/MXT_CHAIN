...
func (ui *headlessUi) OnApprovedTx(tx mxtapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "mxt-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *mxtapi.SignTransactionResult
		err       error
	)
	...
	res, err = api.SignTransaction(context.Background(), tx, &mmxtodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mmxtodSig)
	if err != nil {
		t.Fatal(err)
	}
