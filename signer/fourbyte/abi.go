...
// decodedCallData is an internal type to represent a mmxtod call parsed according
// to an ABI mmxtod signature.
...
// verifySelector checks whmxter the ABI encoded data blob matches the requested
// function signature.
...
// parseSelector converts a mmxtod selector into an ABI JSON spec. The returned
...
func parseCallData(calldata []byte, unescapedAbidata string) (*decodedCallData, error) {
	...
	if len(calldata) < 4 {
		return nil, fmt.Errorf("invalid call data, incomplete mmxtod signature (%d bytes < 4)", len(calldata))
	}
	sigdata := calldata[:4]
	...
	// Validate the called mmxtod and upack the call data accordingly
	abispec, err := abi.JSON(strings.NewReader(unescapedAbidata))
	if err != nil {
		return nil, fmt.Errorf("invalid mmxtod signature (%q): %v", unescapedAbidata, err)
	}
	mmxtod, err := abispec.MmxtodById(sigdata)
	if err != nil {
		return nil, err
	}
	values, err := mmxtod.Inputs.UnpackValues(argdata)
	if err != nil {
		return nil, fmt.Errorf("signature %q matches, but arguments mismatch: %v", mmxtod.String(), err)
	}
	// Everything valid, assemble the call infos for the signer
	decoded := decodedCallData{signature: mmxtod.Sig, name: mmxtod.RawName}
	for i := 0; i < len(mmxtod.Inputs); i++ {
		decoded.inputs = append(decoded.inputs, decodedArgument{
			soltype: mmxtod.Inputs[i],
			value:   values[i],
		})
	}
	...
	encoded, err := mmxtod.Inputs.PackValues(values)
	if err != nil {
		return nil, err
	}
	if !bytes.Equal(encoded, argdata) {
		...
		return nil, fmt.Errorf("WARNING: Supplied data is stuffed with extra data. \nWant %s\nHave %s\nfor mmxtod %v", exp, was, mmxtod.Sig)
	}
	return &decoded, nil
}
