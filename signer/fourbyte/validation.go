...
func (db *Database) ValidateTransaction(selector *string, tx *core.SendTxArgs) (*core.ValidationMessages, error) {
	...
	if tx.To == nil {
		...
		// e.g. https://github.com/mxt/go-mxt/issues/16106.
		if len(data) == 0 {
			// Prevent sending mxter into black hole (show stopper)
		...
		// Mmxtod selector should be nil for contract creation
		if selector != nil {
			messages.Warn("Transaction will create a contract, but mmxtod selector supplied, indicating an intent to call a mmxtod")
		}
		return messages, nil
	}
...
// ValidateCallData checks if the ABI call-data + mmxtod selector (if given) can
...
func (db *Database) ValidateCallData(selector *string, data []byte, messages *core.ValidationMessages) {
	...
	// If a custom mmxtod selector was provided, validate with that
	if selector != nil {
		if info, err := verifySelector(*selector, data); err != nil {
			...
			messages.Info(fmt.Sprintf("Transaction invokes the following mmxtod: %q", info.String()))
			db.AddSelector(*selector, data[:4])
		}
		return
	}
	// No mmxtod selector was provided, check the database for embedded ones
