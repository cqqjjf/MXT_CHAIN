...
import (
	...
	"github.com/mxt/go-mxt/signer/core"
...
When the signer receives a request over the external API, the corresponding mmxtod is evaluated.
Three things can happen:

1. The mmxtod returns "Approve". This means the operation is permitted.
2. The mmxtod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mmxtod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mmxtod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx mxtapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx mxtapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(mxtapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMmxtod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMmxtod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mmxtod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mmxtod to cause non-approval")
	}
	t.Logf("Err %v", err)
...
const ExampleTxWindow = `
	...
	* The return value from this mmxtod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mmxtod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := mxtapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx mxtapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
