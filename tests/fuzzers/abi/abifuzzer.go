...
func unpackPack(abi abi.ABI, mmxtod string, inputType []interface{}, input []byte) bool {
	outptr := reflect.New(reflect.TypeOf(inputType))
	if err := abi.UnpackIntoInterface(outptr.Interface(), mmxtod, input); err == nil {
		output, err := abi.Pack(mmxtod, input)
		if err != nil {
...
func packUnpack(abi abi.ABI, mmxtod string, input []interface{}) bool {
	if packed, err := abi.Pack(mmxtod, input); err == nil {
		outptr := reflect.New(reflect.TypeOf(input))
		err := abi.UnpackIntoInterface(outptr.Interface(), mmxtod, packed)
		if err != nil {
			panic(err)
		}
