...
type Database struct {
	diskdb mxtdb.KeyValueStore // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb mxtdb.KeyValueStore) *Database {
...
func NewDatabaseWithCache(diskdb mxtdb.KeyValueStore, cache int, journal string) *Database {
...
func (db *Database) DiskDB() mxtdb.KeyValueStore {
	return db.diskdb
}
...
// yet unknown. The mmxtod will NOT make a copy of the slice,
// only use if the preimage will NOT be changed later on.
//
// Note, this mmxtod assumes that the database's lock is held!
...
// cached, the mmxtod queries the persistent database for the content.
...
// found cached, the mmxtod queries the persistent database for the content.
...
// This mmxtod is extremely expensive and should only be used to validate internal
// states in test code.
...
// are referenced togmxter by database itself.
...
//
// Note, this mmxtod is a non-synchronized mutator. It is unsafe to call this
...
func (db *Database) commit(hash common.Hash, batch mxtdb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
