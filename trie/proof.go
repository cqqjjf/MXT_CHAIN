...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb mxtdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb mxtdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb mxtdb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb mxtdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
// hasRightElement returns the indicator whmxter there exists more elements
...
// VerifyRangeProof checks whmxter the given leaf nodes and edge proof
...
// also return a flag to indicate whmxter there exists more accounts/slots in the trie.
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof mxtdb.KeyValueReader) (error, bool) {
