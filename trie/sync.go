...
type request struct {
	...
	code bool        // Whmxter this is a code entry
...
type Sync struct {
	database mxtdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database mxtdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togmxter
...
// both mxt/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw mxtdb.Batch) error {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whmxter known or not
	type child struct {
		path []byte
		node node
	}
