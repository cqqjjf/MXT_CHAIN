...
func NewSyncBloom(memory uint64, database mxtdb.Iteratee) *SyncBloom {
...
func (b *SyncBloom) init(database mxtdb.Iteratee) {
...
func (b *SyncBloom) Contains(hash []byte) bool {
	...
	if atomic.LoadUint32(&b.inited) == 0 {
		// We didn't load all the trie nodes from the previous run of Gmxt yet. As
