...
func init() {
	spew.Config.Indent = "    "
	spew.Config.DisableMmxtods = false
}
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"sommxtingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func (s *spongeDb) NewBatch() mxtdb.Batch                    { return &spongeBatch{s} }
...
func (s *spongeDb) NewIterator(prefix []byte, start []byte) mxtdb.Iterator { panic("implement me") }
...
func (b *spongeBatch) Replay(w mxtdb.KeyValueWriter) error { return nil }
...
// to check whmxter changes to the trie modifies the write order or data in any way.
